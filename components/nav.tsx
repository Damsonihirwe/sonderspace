...
export function Nav() {
  ...
  return <header className="fixed left-0 top-0 z-50 w-full mix-blend-difference text-paper">
    <div className="mx-auto flex max-w-[1440px] items-center justify-between px-5 py-5 md:px-10">
      <Link href="/" className="font-display text-2xl tracking-tight">SONDER<span className="text-signal">.</span></Link>
      <button aria-label="Toggle menu" onClick={() => setOpen(!open)} className="flex items-center gap-3 font-mono text-[10px] uppercase tracking-[.18em] md:hidden">{open ? 'Close' : 'Menu'} <span className="h-2 w-2 bg-signal" /></button>
